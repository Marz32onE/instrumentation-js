...
class TracedSubscription implements Subscription {
  ...
  get callback(): import('@nats-io/transport-node').MsgCallback<Msg> {
...
export class OtelNatsConn {
  ...
  status(): AsyncIterable<import('@nats-io/transport-node').Status> {
    return this.#nc.status();
  }

  stats(): import('@nats-io/transport-node').Stats {
  ...
  get info(): import('@nats-io/transport-node').ServerInfo | undefined {
    return this.#nc.info;
  }
