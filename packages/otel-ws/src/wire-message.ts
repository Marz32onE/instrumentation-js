...
export function deserializeMessage<T = unknown>(raw: string): ParsedWireMessage<T> {
  try {
    ...
    diag.debug('[otel-ws] received non-envelope message, delivering data without trace context extraction');
    return { data: parsed as T };
  } catch (err) {
    diag.debug('[otel-ws] deserializeMessage: JSON.parse failed, treating as raw string', { error: String(err) });
